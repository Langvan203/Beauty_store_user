...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Tạo userName bằng cách kết hợp firstName và lastName
    ...
    try {
      ...
      if (data.status === 1 || response.ok) {
        ...
        // Đăng ký thất bại
        alert(`Đăng ký thất bại: ${data.des || "Đã có lỗi xảy ra"}`)
      }
    } catch (error) {
      ...
      <Card className="w-full max-w-md">
        ...
        <CardFooter className="flex flex-col">
          <p className="text-center text-sm text-gray-600">
            Đã có tài khoản?{" "}
            <Link href="/auth/login" className="text-pink-500 hover:underline font-medium">
              Đăng nhập
            </Link>
          </p>
        </CardFooter>
